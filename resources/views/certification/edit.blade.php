    ...
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('certifications.update', $certification->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="date" name="expiration_at" id="expiration_at" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ ($certification->expiration_at) ? $certification->expiration_at->toDateString() : old('expiration_at') }}" min="{{ now()->toDateString() }}" />
                            @error('expiration_at')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
