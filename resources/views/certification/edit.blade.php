    ...
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('certifications.update', $certification->id) }}">
                    @csrf
                    @method('put')
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="short" id="short" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('short', $certification->short) }}" />
                            @error('short')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="identifier" id="identifier" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('identifier', $certification->identifier) }}" />
                            @error('identifier')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="date" name="issued_at" id="issued_at" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('issued_at', $certification->issued_at->toDateString()) }}" max="{{ now()->toDateString() }}" />
                            @error('issued_at')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="date" name="expiration_at" id="expiration_at" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('expiration_at', ($certification->expiration_at ? $certification->expiration_at->toDateString():'')) }}" min="{{ now()->toDateString() }}" />
                            @error('expiration_at')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select name="company_id" id="company_id" class="form-multiselect block rounded-md shadow-sm mt-1 block w-full">
                                @foreach($companies as $id => $company)
                                    <option value="{{ $id }}"{{ ($id == old('company_id', $certification->company_id)) ? ' selected' : '' }}>{{ $company->name }}</option>
                                @endforeach
                            </select>
                            @error('company_id')
                                <p class="text-sm text-red-600">{{ $message }}</p>
