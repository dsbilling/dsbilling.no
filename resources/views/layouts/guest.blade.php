...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="h-screen min-h-full {{ env('APP_DEBUG') ? 'debug-screens' : '' }}">
        <div class="min-h-full pb-8 font-sans antialiased text-gray-900 bg-white dark:bg-gray-900">
            <nav x-data="{ open: false }" class="px-4 bg-white dark:bg-gray-900 sm:px-0">
                <div class="max-w-4xl mx-auto">
                    <div class="flex items-center justify-between w-full h-16">
                        <div class="flex items-center">
                            <a class="flex" href="{{ route('home') }}">
                                <div class="flex items-center font-semibold tracking-wider text-gray-900 uppercase transition-colors dark:text-white hover:text-orange-400 focus:text-orange-400 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500 focus-visible:ring-opacity-60 focus-visible:outline-none focus:outline-none"> {{ config('app.name') }}</div>
                            </a>
                        </div>
